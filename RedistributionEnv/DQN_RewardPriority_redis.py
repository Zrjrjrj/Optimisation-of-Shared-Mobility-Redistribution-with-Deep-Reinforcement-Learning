...
class RewardBasedPrioritizedReplayBuffer:
    def __init__(self, memory, alpha=0.8, beta=0.4):
    ...
    def add(self, transition, final_reward):
        ...
        # else:
        #     # 如果缓冲区已满，只替换负reward的经验
        #     replaced = False
        #     while not replaced:
        #         current_experience = self.buffer[self.pos]
        #         if current_experience.transition.reward <= -10:  # 只替换负reward的经验
...
if __name__ == "__main__":
    ...
    model_dir = "DQN_RewardPriority-50000_models"
    plot_dir = "DQN_RewardPriority-50000_plots"
    ...
    for e in range(episodes):
        ...
        if (e + 1) % 2500 == 0 or e == episodes - 1:
            agent.save(os.path.join(model_dir, f"dqn_RewardPriority_redistribution_{e + 1}.pth"))
            ...
            plt.title('Smoothed DQN Reward Priority Training Rewards')
            plt.grid(True)
            ...
            plt.xlabel('Episode')
            plt.ylabel('Total Reward')
            plt.title('Raw DQN Reward Priority Training Rewards')
            plt.grid(True)
    ...
    plt.xlabel('Episode')
    plt.ylabel('Total Reward')
    plt.title('Final Smoothed DQN Reward Priority Training Rewards')
    ...
    plt.xlabel('Episode')
    plt.ylabel('Total Reward')
    plt.title('Final Raw DQN Reward Priority Training Rewards')
    plt.text(0.5, 0.01, f'Total training time: {total_training_time / 1000:.2f} seconds', horizontalalignment='center',
             verticalalignment='center', transform=plt.gca().transAxes)
    plt.grid(True)
